...
@select.route('/banji', methods=['POST'])
# @login_required
def banji():
    if request.method == 'POST':
        ...
        "127.0.0.1:5000:/banji?class_name=17dashuju&semester=d&year=2018&college=大数据与人工智能"
        ...
        college_value = 'select avg(political_edu),avg(physical_heal),avg(innovation_entrep),avg(technical_skills),avg(volunte),avg(human_art),avg(zh_theory) from bigdata WHERE grade="{}" {};'.format(
            grade, filter_str)
        class_value = 'select avg(political_edu),avg(physical_heal),avg(innovation_entrep),avg(technical_skills),avg(volunte),avg(human_art),avg(zh_theory) from bigdata WHERE gk_class="{}" {};'.format(
            class_name, filter_str)
        ...
        # 4---
        student_name = ' select stu_name from bigdata where gk_class="{}" and semester="第二学期" and year="{}";'.format(
            class_name, year)
        stu_name_app = []
        ...
        for key, value in indexs.items():
            name_app = []
            score_app = []
            all_sql = 'select stu_name,{} from bigdata where gk_class="{}" {filter_str} order by {} desc limit 5;'.format(
                key, class_name, key, filter_str=filter_str)
            db.execute(all_sql)
            allName = db.fetchall()
        ...
        total_sql = 'select count(stu_name),CASE ' \
                    ...
                    'when score > 30 and score <= 40 THEN "31-40" ' \
                    'when score > 40 and score <=50 THEN "41-50"  ' \
                    'when score>50 THEN "50以上"  end;'.format(class_name, filter_str=filter_str)
        ...
        return jsonify(
            {"classjbCard": classjbCard, "classCard": classCard, "suchindexscores": suchindexscores,
             "students": students,
             "topstudent": topstudent, "totalscores": totalscores})


@select.route('/geren', methods=['POST'])
# @login_required
def geren():
    if request.method == 'POST':
        formdata = request.json
        ...
        print(stu_name)
        # 1--
        stu_score = 'select zh_score from bigdata where stu_name="{}" and semester="第一学期" and year="{}";'.format(
            stu_name, year)
        print(stu_score)
        ...
        # 1_1--
        stu_score_2 = 'select zh_score from bigdata where stu_name="{}" and semester="第二学期" and year="{}";'.format(
            stu_name, year)
        db.execute(stu_score_2)
        term2_avlscore = db.fetchall()[0][0]
